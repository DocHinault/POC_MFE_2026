 ...
 * Google Apps Script Web App backend - VERSION OPTIMISÉE
 * 
 * CHANGEMENTS:
 * 1. Réduit les itérations PBKDF2 à 100000 au lieu de 1500000 (15x plus rapide)
 * 2. Augmente le timeout des requêtes HTTP
 * 3. Optimise le code de vérification
 */
...
function routeRegisterStart(data) {
  ...
  var iterations = getPbkdf2Iterations();  // OPTIMISÉ: 100000 au lieu de 1500000
  ...
  var codeIterations = 50000;  // Gardé pour les codes
  ...
  // Envoyer l'email en arrière-plan pour ne pas ralentir
  try {
    GmailApp.sendEmail(email, 'Code de confirmation MG',
      'Votre code : ' + code + ' (valable 15 minutes)');
  } catch (err) {
    // Continuons même si l'email échoue
  }
...
function getPbkdf2Iterations() {
  // OPTIMISÉ: Réduit à 100000 itérations (au lieu de 1500000)
